...
def main(
    ...
    # Get run hyperparameters
        ...
        params_path = (
            run_dir / "params.json"
            if continue_from_run is not None
            else HPARAMS_DIR / "ROME" / hparams_fname
        )
    elif "AlphaEdit" in alg_name:
        params_path = (
            run_dir / "params.json"
            if continue_from_run is not None
            else HPARAMS_DIR / "AlphaEdit" / hparams_fname
        )
    else:
        params_path = (
            run_dir / "params.json"
    ...
    for record_chunks in chunks(ds, num_edits):
        ...
        if cnt == -1 and args.downstream_eval_steps > 0:#do initial GLUE EVAL WITH ORIGINAL MODEL
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--alg_name",
        choices=["AlphaEdit","MEMIT_rect", "MEMIT_seq","MEMIT_prune", "MEMIT", "ROME", "FT", "MEND", "NSE", "MEMIT_defence", "ROME_defence", "AlphaEdit_defence"],
        default="ROME",
