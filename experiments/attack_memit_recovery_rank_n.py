...
def create_name_database():
    """
    创建一个包含前2000个名字的数据库，确保包含目标主语
    """
    subjects, _ = load_dataset_data(limit=2000)
    
    # 确保特定的目标名字在里面 (根据具体 Case 可能需要修改)
...
def get_activation_vector_for_name(
    ...
    """
    获取指定名字在目标层的激活向量 (k_cand)
    """
    context_templates = [template]
    words = [name]
    
    # get_module_input_output_at_words 返回 (inputs, outputs)
    # 我们需要 inputs (进入 MLP 的向量)
...
def get_activation_vector_for_prompt(
    ...
    Args:
        ...
        tok: 分词器
        subject: 主语
        prompt_template: 提示词模板（包含{}占位符）
        layer: 目标层
        module_template: 模块模板
        fact_token_strategy: 策略（如"subject_last"）
    
    Returns:
        activation_input: 激活输入向量
    """
    # 填充提示词模板
    full_prompt = prompt_template.format(subject)
    
    # 使用subject_last策略获取激活向量
...
def main():
    ...
    # 1. 配置参数
    case_id = "0" # 根据实际情况修改
    target_rank = 10 # 假设秩为 10
    
    # 注意路径变化：multi_case_edit_memit_amount
    ...
    config = SimpleNamespace(
        ...
        final_layer_to_attack=4, # 目标层
        
        target_rank=target_rank, # 关键参数：指定秩 N
    ...
    knowledge_template = "The US dollar {} earns each year is" # 根据 case_id 0 调整
    
    # 2. 检查文件
    if not Path(config.delta_file_path).exists():
        print(f"Error: Cannot find edit amount file: {config.delta_file_path}")
        # 为了演示，如果找不到 multi_case 目录，尝试回退到 orth_defence 目录或者报错
        # config.delta_file_path = f"orth_defence_edit_memit_amount/edit_amounts_batch_case_{case_id}.pt"
        # config.kr_ground_truth_path = f"orth_defence_edit_memit_amount/kr_ground_truth_case_{case_id}.pt"
        # print(f"Trying fallback path: {config.delta_file_path}")
        return
    ...
    try:
        # run_attack_simple_k_r_CORRECT 返回:
        # k_attack_result (Rank>1 时为子空间正交基)
    ...
    Q_basis = Q_basis.to("cuda").double() # 确保在 GPU 上且精度匹配
    ...
    # 6. [新增功能] 对得分前target_rank的主语，计算所有候选提示词的投影得分
    print("\n" + "=" * 80)
    ...
    Q_basis = Q_basis.to("cuda").double()  # 确保在GPU上
    ...
    for subject_idx, subject in enumerate(top_subjects, 1):
        ...
        for prompt_idx, prompt_template in enumerate(prompt_library):
            try:
                ...
                # 计算投影系数: Q_basis.T @ activation_input
                # Q_basis: (D, Rank), activation_input: (D,)
                proj_coeffs = Q_basis.T @ activation_input  # (Rank,)
                
                # 计算投影得分: ||proj_coeffs|| / ||activation_input||
                proj_norm = torch.norm(proj_coeffs)
                ...
                prompt_scores.append({
                    ...
                    "proj_coeffs": proj_coeffs.cpu().tolist()  # 保存投影系数（转为列表）
                })
        ...
        print("-" * 120)
        for rank, item in enumerate(prompt_scores[:20], 1):  # 打印前20个
